...
def detect_img_link(prod):
    # يدعم أسماء الحقول common للصورة
    for key in ['image_link', 'imagelink', 'imagelinks', 'image']:
        val = prod.get(key, '')
        if val:
            return val
    return ''

def make_product_schema(prod, img_url):
    return json.dumps({
        ...
        "offers": {
            "@type": "Offer",
            "priceCurrency": "AED",
            "price": prod.get("price", "0").replace(" AED","").replace("AED ",""),
            "url": prod.get("link", ""),
...
if not os.path.exists('.git'):
    print('\n[خطأ] يجب تشغيل السكربت من داخل مجلد الريبو الأصلي\n')
    sys.exit(1)
if not os.path.isfile("products.json"):
    print('\n[خطأ] ملف المنتجات products.json غير موجود!\n')
    sys.exit(2)
...
if os.path.isfile("products-list.html"):
    os.remove("products-list.html")
print("توليد ملفات المنتجات ...")
...
product_template_str = """<!DOCTYPE html>
...
<a class="wa-btn" href="https://wa.me/201110760081?text=اريد%20$wa_title%20(%20$price%20)%20عدد%20:%20[اكتب%20رقم%20الكمية]%20العنوان%20:%20[اكتب%20العنوان]" target="_blank">اطلب عبر واتساب</a>
<a class="wa-btn" style="background:#3a80f7;" href="$link" target="_blank">شاهد المنتج على الموقع الرسمي</a>
</div>
</div>
$footer
</body>
...
grid_template_str = """<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
<meta charset="UTF-8">
...
for product in products:
    ...
    img_url = detect_img_link(product)
    desc_short = product.get('description', '').strip()
    if not title or not price or not link or not img_url:
        print(f"[تخطٍ] منتج لم يُولّد: title='{title}'  price='{price}' img='{img_url}' link='{link}'")
        continue
    safe_title = clean_filename(title)
    filename = f"{safe_title}.html"
    desc_long = f"{title} منتج حصري بجودة عالية وسعر تنافسي."
    schema_json = make_product_schema(product, img_url)
    page_content = product_template.substitute(
        title=html.escape(title),
        wa_title=html.escape(title),
        price=price,
    ...
    <a href="{output_dir}/{filename}" class="prod-card">
        <img src="{html.escape(img_url)}" alt="{html.escape(title)}">
...
print(f"\nتم توليد {count} صفحة منتج.")

try:
    subprocess.run("git add .", shell=True, check=True)
    subprocess.run(f'git commit -m "auto: إعادة توليد جميع منتجات وصفحات المتجر"', shell=True, check=True)
    subprocess.run("git push", shell=True, check=True)
    print("✅ تم رفع جميع الملفات دفعة واحدة!")
except Exception as e:
    print("⚠️ خطأ في git:", e)
